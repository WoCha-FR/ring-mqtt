...
export default new class Mqtt {
    ...
    async init() {
        try {
            ...
            console.log(new Date().toLocaleString() + ': Attempting connection to MQTT broker...')
            ...
            console.log(new Date().toISOString() + ': Could not authenticate to MQTT broker. Please check the broker and configuration settings.')
            process.exit(1)
        }
    }
    ...
    start() {
        ...
        this.client.on('reconnect', () => {
            if (this.connected) {
                ...
                console.log(new Date().toISOString() + ': Connection to MQTT broker lost. Attempting to reconnect...')
            } else {
        ...
        this.client.on('message', (topic, message) => {
            ...
            if (topic === utils.config().hass_topic) {
