...
export default new class Main {
    ...
    async init(generatedToken) {
        ...
        // For the Jeedom addon, Web UI is always started
    ...
    async attemptRingConnection(generatedToken) {
        if (!await ring.init(state, generatedToken)) {
            ...
            console.log(new Date().toISOString() + ': Failed to connect to Ring API using saved token, generate a new token using the Web UI.')
            console.log(new Date().toISOString() + ': or wait 60 seconds to automatically retry authentication using the existing token.')
            webService.start(state.data.systemId)
            await utils.sleep(60)

            if (!ring.client) {
                debug(chalk.yellow('Retrying authentication with existing saved token...'))
                console.log(new Date().toISOString() + ': Retrying authentication with existing saved token...')
                this.init()
            }
        }
    }
    ...
    handleNoToken() {
        if (process.env.RUNMODE === 'addon') {
            ...
            console.log(new Date().toISOString() + ': No refresh token was found in the state file, use the Web UI at http://<host_ip_address>:55123/ to generate a token.')
        }
    }
}