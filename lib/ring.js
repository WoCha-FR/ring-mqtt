...
export default new class RingMqtt {
    constructor() {
        ...
        utils.event.on('ha_status', async (topic, message) => {
            debug('Jeedom state topic '+topic+' received message: '+message)
            console.log(new Date().toISOString() + 'Jeedom state topic '+topic+' received message: '+message)
            if (message === 'refresh') {
                // Republish devices and state if restart of HA is detected
                if (this.republishCount > 0) {
                    debug('Jeedom refresh detected during existing republish cycle')
                    debug('Resetting device config/state republish count')
                    this.republishCount = 6
                } else {
                    debug('Jeedom refresh detected, resending device config/state in 15 seconds')
    ...
    async init(state, generatedToken) {
        ...
        if (this.client) {
            ...
            try {
                ...
                console.log(new Date().toISOString() + ': Successfully established connection to Ring API')
    ...
    async initRingData() {
        ...
        console.log(new Date().toISOString() + ': This account has access to the following locations:')
        locations.map(function(location) {
            debug('           '+chalk.green(location.name)+chalk.cyan(` (${location.id})`))
            console.log('           ' + new Date().toISOString() + ': '+ location.name + ' ' + location.id)
        })
        debug(' '.repeat(90))
        ...
        console.log(new Date().toISOString() + ': Starting Device Discovery...')
        ...
        console.log(new Date().toISOString() + ': Device Discovery Complete!')
